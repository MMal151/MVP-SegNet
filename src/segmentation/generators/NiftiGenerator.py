...
from src.utils.ProcessUtils import normalize_img
from src.utils.CommonUtils import str_to_list, str_to_tuple
...
class Nifti2DGenerator(Nifti3DGenerator):
    def __init__(self, cfg, df, mode):
        super().__init__(cfg, df, mode)
    ...
    def load_batch(self, idx):
        ...
        for i, batch in enumerate(batches):
            ...
            if len(batch) == 3:
                axial_axis = batch[2][2]
                img = np.rot90(img[:, :, axial_axis])
                lbl = np.rot90(lbl[:, :, axial_axis])
